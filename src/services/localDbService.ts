...
const USER_PRESETS_STORE_NAME = 'user_presets';
...
export const getUserProfile = async (userId: string): Promise<User | null> => {
    ...
    return sanitizeUserProfileUrls(user || null);
};
...
export const getAllUserProfiles = async (): Promise<User[]> => {
    ...
    return users
        ...
        .map(user => {
            ...
            const sanitized = { ...user, joinDate: validDate };
            return sanitizeUserProfileUrls(sanitized) as User;
        });
};

export const updateUserProfile = async (userId: string, data: Partial<User>): Promise<void> => {
    const existing = await dbGet<User>(USER_STORE_NAME, userId) || {} as User;
    await dbPut(USER_STORE_NAME, { ...existing, ...data, id: userId });
};

export const checkSlugAvailability = async (slug: string, requestingUserId: string): Promise<boolean> => {
    return true; // Local mode always assumes available to prevent offline lockout
};
...
export const deleteUserProfile = async (userId: string): Promise<void> => {
    await dbDelete(USER_STORE_NAME, userId);
};
...
export const deleteChatHistory = async (userId: string): Promise<void> => {
    await dbDelete(CHAT_HISTORY_STORE_NAME, userId);
};
...
export const deleteMedia = async (userId: string, mediaId: string, targetCollection?: string): Promise<void> => {
    await dbDelete(MEDIA_STORE_NAME, mediaId);
};
export const getMediaById = async (mediaId: string, userId: string): Promise<Media | null> => {
    const media = await dbGet<Media>(MEDIA_STORE_NAME, mediaId);
    return media || null;
};
...
export const getUserPresets = async (userId: string): Promise<any[]> => {
    return await dbGetAll(USER_PRESETS_STORE_NAME);
};
export const saveUserPreset = async (userId: string, preset: any): Promise<void> => {
    await dbPut(USER_PRESETS_STORE_NAME, preset);
};
export const deleteUserPreset = async (userId: string, presetId: string): Promise<void> => {
    await dbDelete(USER_PRESETS_STORE_NAME, presetId);
};
...
// --- MEMORIAL AIRLOCK (STUBS FOR LOCAL MODE) ---
