...
import { Loader2, DownloadCloud, ExternalLink, X, StopCircle, HelpCircle } from 'lucide-react';
...
interface ImportModalProps {
    ...
    errorMsg: string;
    onLaunch: () => void;
    onConnect: () => void; // [ZEN] New handler
    onCancel: () => void;
}

export const ImportModal: React.FC<ImportModalProps> = ({ isOpen, step, errorMsg, onLaunch, onConnect, onCancel }) => {
            ...
            <div className="bg-[#0f1219] max-w-md w-full rounded-2xl border border-blue-500/30 shadow-[0_0_50px_rgba(59,130,246,0.2)] p-8 text-center">
                ...
                {step === 'ready' && (
                    <div className="py-4">
                        <div className="flex items-center justify-center gap-2 mb-6">
                            <p className="text-slate-300">Link established.</p>
                            {/* UX Tooltip */}
                            <div className="group relative">
                                <HelpCircle size={14} className="text-blue-400 cursor-help" />
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2 bg-slate-800 text-[10px] text-slate-300 rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity border border-blue-500/20 shadow-xl z-20">
                                    Google requires the picker to open in a new window to protect your privacy and credentials.
                                </div>
                            </div>
                        </div>
                        <button
                ...
                {step === 'error' && (
                    <div className="py-4">
                        ...
                        {errorMsg === 'GOOGLE_AUTH_REQUIRED' ? (
                            <button
                                onClick={onConnect}
                                className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl flex items-center justify-center gap-2 mb-3 shadow-lg"
                            >
                                <ExternalLink size={18} /> CONNECT GOOGLE PHOTOS
                            </button>
                        ) : null}
