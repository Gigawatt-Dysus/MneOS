...
// Interface matches Lucide-React props for drop-in compatibility
interface IconProps {
    size?: number | string;
    className?: string;
    color?: string;
...
export const DeepDiveIcon: React.FC<IconProps> = ({ size = 24, className = "", color = "currentColor" }) => (
    <svg 
        height={size} 
        width={size} 
        viewBox="0 0 24 24" 
        ...
        className={className}
        fill={color} // Use current text color for hover effects
        // Removed hardcoded stroke/fill so it inherits from Tailwind classes
    >
