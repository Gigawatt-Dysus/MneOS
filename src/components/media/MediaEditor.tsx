...
import { TEMPORAL_SHOEBOX_DATE, TEMPORAL_SHOEBOX_YEAR } from '../../types/constants';
...
export const MediaEditor: React.FC<MediaEditorProps> = ({
    ...
    const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (val) {
            ...
            setFormData(prev => ({
                ...
                year: d.getFullYear(),
                datePrecision: prev.datePrecision === 'unknown' ? 'exact' : prev.datePrecision
            }));
        }
    };

    const toggleShoebox = () => {
        const isCurrentlyShoebox = formData.datePrecision === 'unknown' || formData.year === TEMPORAL_SHOEBOX_YEAR;
        
        if (isCurrentlyShoebox) {
            // Restore to now
            const now = new Date();
            ...
            setFormData(prev => ({
                ...prev,
                logicalDate: TEMPORAL_SHOEBOX_DATE,
                year: TEMPORAL_SHOEBOX_YEAR,
    ...
    return (
        <div className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-xl flex items-center justify-center p-4 md:p-8 animate-in fade-in zoom-in-95">
            <div className="bg-[#0f1219] w-full max-w-7xl h-[90vh] rounded-3xl border border-white/10 shadow-2xl flex flex-col md:flex-row overflow-hidden relative">
                ...
                {/* --- RIGHT: CONTROL PANEL --- */}
                <div className="w-full md:w-[500px] bg-[#13161f] flex flex-col h-full border-l border-white/10 shadow-2xl z-20">
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
                        ...
                        {activeTab === 'details' && (
                            <div className="space-y-6 animate-in slide-in-from-right-2 fade-in">
                                ...
                                <div className={`border p-4 rounded-xl transition-all ${formData.datePrecision === 'unknown' ? 'bg-amber-500/10 border-amber-500/30' : 'bg-violet-500/5 border-violet-500/20'}`}>
                                    <div className="flex items-center justify-between mb-2">
                                        <label className={`text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 ${formData.datePrecision === 'unknown' ? 'text-amber-400' : 'text-violet-400'}`}>
                                            <CalendarDays size={12} /> {formData.datePrecision === 'unknown' ? 'Shoebox Anchor' : 'Timeline Sort Date'}
                                        </label>
                                        <button 
                                            onClick={toggleShoebox}
                                            className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded border transition-all ${
                                                formData.datePrecision === 'unknown' 
                                                ? 'bg-violet-500/20 border-violet-500/30 text-violet-400 hover:bg-violet-500/30' 
                                                : 'bg-amber-500/20 border-amber-500/30 text-amber-400 hover:bg-amber-500/30'
                                            }`}
                                        >
                                            {formData.datePrecision === 'unknown' ? 'Restore to Timeline' : 'Relegate to Shoebox'}
                                        </button>
                                    </div>
                                    {formData.datePrecision === 'unknown' ? (
                                        <div className="w-full bg-[#0a0c10] border border-amber-500/30 rounded-lg px-3 py-2 text-sm text-amber-400 font-mono flex items-center gap-2">
                                            <Archive size={14} /> YEAR 5000 (UNDATED)
                                        </div>
                                    ) : (
                                        <input
                                            type="datetime-local"
                                    ...
                                    <p className={`text-[10px] mt-2 ${formData.datePrecision === 'unknown' ? 'text-amber-400/60' : 'text-violet-400/60'}`}>
                                        {formData.datePrecision === 'unknown' 
                                            ? 'This item lives in the Shoebox until a date is identified.' 
                                            : 'Adjusting this moves the item in the Time Vortex.'}
                                    </p>
                                </div>
