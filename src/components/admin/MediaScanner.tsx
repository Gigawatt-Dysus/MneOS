...
import { collection, getDocs, doc, updateDoc } from '../../services/sovereignDbAdapter';
...
export const MediaScanner: React.FC<MediaScannerProps> = ({ allTags: initialTags, userId }) => {
    ...
    const [mode, setMode] = useState<'vision' | 'repair'>('vision');
    ...
    // [ZEN EWO 001] 'faces' mode removed - Azure integration pending
    const loadQueue = async (targetMode: 'vision' | 'repair') => {
        setIsScanning(true);
        setMode(targetMode);
        setQueue([]);
    ...
    const processRepairItem = async (item: Media, currentTags: Tag[]): Promise<any> => {
        ...
        if (item.description) {
            ...
            try {
                const res = await callXAI('grok-4.3', [{ role: 'user', content: prompt }], "Output JSON only.");
    ...
    const startProcessing = async () => {
        ...
        // [ZEN EWO 001] Client-side face recognition removed - using cloud Vision only
        addLog("🔄 Fetching Tag Data...");
        let freshTags: Tag[] = [];
        try {
            ...
            addLog(`✅ Retrieved ${freshTags.length} tags.`);
        } catch (e) {
            console.error("Tag fetch failed", e);
            addLog("❌ Tag Fetch Failed.");
            setIsScanning(false);
            return;
        }
        ...
        for (const item of queue) {
            ...
            try {
                ...
                // [ZEN EWO 001] Face recognition removed - Azure integration pending
        ...
        <div className="bg-slate-900 border border-cyan-500/30 rounded-xl p-6 shadow-2xl">
            ...
            <div className="flex gap-4 justify-end flex-wrap">
                {!isScanning && queue.length === 0 && (
                    <>
                        {/* [ZEN EWO 001] Face scanning disabled - Azure integration pending */}
                        <button disabled className="px-4 py-2 bg-slate-800 text-slate-500 rounded-lg font-bold flex items-center gap-2 cursor-not-allowed opacity-50">
                            <User size={16} /> Scan Faces (Coming Soon)
                        </button>
