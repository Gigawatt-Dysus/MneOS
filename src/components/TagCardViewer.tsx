...
import { Search, Grid, List, User, Tag as TagIcon, MapPin, Package, Calendar, Dog, Edit, MessageSquare, Trash2, ExternalLink, Plus, Image as ImageIcon, X, Loader2, Brain } from 'lucide-react';
import type { Tag, Media, PersonTag, PetTag, PlaceTag, ConceptTag } from '../types';
...
interface TagGalleryProps {
    ...
    currentUser?: GIGIUser | null; // [ZEN FIX] Added missing prop
    addToast: (msg: string, type: 'success' | 'error') => void; // [ZEN FIX] Added missing prop
}

import type { User as GIGIUser } from '../types';
...
export const TagGallery: React.FC<TagGalleryProps> = ({
    tags, media, tagBeingDeleted, onEditTag, onCreateTag, onDeleteTag, onDiscuss, currentUser, addToast, onReplaceTag
}) => {
    const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
    const [searchQuery, setSearchQuery] = useState('');
    const [filterType, setFilterType] = useState<string>('all');
    const [selectedTag, setSelectedTag] = useState<Tag | null>(null);
    const [showInspector, setShowInspector] = useState(false); // [ZEN] Inspector State
    const [inspectedTag, setInspectedTag] = useState<PersonTag | null>(null); // [ZEN FIX] Added missing state
                ...
                <div className={`h-24 ${getTagColor(tag.type)} relative overflow-hidden`}>
                    {imageUrl ? (
                        ...
                        <div className="absolute inset-0 flex items-center justify-center opacity-20">
                            {tag.type === 'concept' ? <Brain size={64} /> : <TagIcon size={64} />}
                        </div>
                    )}
                ...
                <div className="p-4 relative flex-1 flex flex-col pt-12">
                    ...
                    <div className="absolute top-2 right-4">
                        <span className="text-[10px] uppercase font-bold bg-slate-800 text-slate-400 px-2 py-0.5 rounded border border-slate-700 tracking-wider">
                            {tag.type === 'concept' ? `CONCEPT: ${((tag as any).metadata?.flavor || 'General').toUpperCase()}` : getRelationshipLabel(tag)}
                        </span>
                    </div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-3xl shadow-2xl flex flex-col max-h-[90vh] isolate" onClick={e => e.stopPropagation()}>
                    ...
                    <div className={`h-32 ${getTagColor(selectedTag.type)} relative shrink-0 rounded-t-2xl z-20`}>
                        ...
                        <button
                            ...
                            className="absolute top-14 right-8 p-2 bg-black/30 hover:bg-black/50 rounded-full text-white transition-colors z-20"
                        >
                            <X size={20} />
                        </button>
                    ...
                    <div className="px-8 pb-8 pt-32 flex flex-col flex-1 overflow-y-auto custom-scrollbar relative z-0">
                        <div className="flex justify-end mb-6">
                            <div className="flex gap-3">
                                ...
                                {/** [ZEN DEBUG] Forced Button Render */}
                                <button
                                    onClick={() => {
                                        console.log("[Inspector] Toggling. Current Tag:", selectedTag);
                                        setShowInspector(!showInspector);
                                        if (selectedTag && selectedTag.type === 'person') {
                                            setInspectedTag(selectedTag as PersonTag);
                                        } else {
                                            setInspectedTag(null);
                                        }
                                    }}
                                    className={`px-4 py-2 rounded-lg font-bold text-sm flex items-center gap-2 border border-slate-700 transition-colors ${showInspector ? 'bg-pink-600 text-white' : 'bg-slate-800 text-pink-400 hover:text-pink-300'}`}
                                >
                                    <Package size={16} /> Ancestry DEBUG
                                </button>
                            </div>
                        </div>
                        ...
                        <div>
                            <div className="mb-6">
                                ...
                                {/* [ZEN] Markdown Helper Component */}
                                <div className="text-lg text-slate-300 leading-relaxed font-light">
                                    {(selectedTag.description || "No detailed description recorded.").split('\n').map((line, i) => (
                                        <p key={i} className="mb-2">
                                            {/* [ZEN] Robust Markdown Parser V3 */}
                                            {(() => {
                                                // Tokenize by splitting on BOLD (**...**) then ITALIC (*...*)
                                                // We accept *italics* with any content inside except *
                                                ...
                                                return tokens.map((part, j) => {
                                                    ...
                                                    if (part.startsWith('*') && part.endsWith('*') && part.length > 2) {
                                                        // [ZEN] Force italic using plain span + inline style to guarantee override
                                                        // Tailwind 'italic' can be reset by default styles, inline usually wins.
                                                        return <span key={j} className="text-slate-200" style={{ fontStyle: 'italic' }}>{part.slice(1, -1)}</span>;
                                                    }
                            ...
                            <div className="bg-slate-950/50 rounded-xl p-6 border border-slate-800 mb-8">
                                ...
                                <div className="grid grid-cols-2 gap-y-6 gap-x-8">
                                        ...
                                        <>
                                            <div>
                                                ...
                                                <span className="text-white font-medium">
                                                    {formatLifeOSDate((selectedTag as PersonTag).metadata.dates?.birth, (selectedTag as PersonTag).metadata.dates?.birthPrecision || 'day')}
                                                </span>
                                                {(selectedTag as PersonTag).metadata.birthPlace && (
                                                    <div className="text-xs text-slate-400 mt-0.5 ml-1 flex items-center gap-1">
                                                        <MapPin size={10} /> {(selectedTag as PersonTag).metadata.birthPlace}
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            {/* Death Info */}
                                            {((selectedTag as PersonTag).metadata.dates?.death || (selectedTag as PersonTag).metadata.deathPlace) && (
                                                <div className="col-span-2 mt-2 pt-2 border-t border-slate-800/50">
                                                    ...
                                                    <div className="flex gap-4">
                                                        {(selectedTag as PersonTag).metadata.dates?.death && (
                                                            <span className="text-slate-300 font-medium text-sm">
                                                                {formatLifeOSDate((selectedTag as PersonTag).metadata.dates?.death, (selectedTag as PersonTag).metadata.dates?.deathPrecision || 'day')}
                                                            </span>
                                                        )}
                                                        {(selectedTag as PersonTag).metadata.deathPlace && (
                                                            <span className="text-slate-400 text-xs flex items-center gap-1">
                                                                <MapPin size={10} /> {(selectedTag as PersonTag).metadata.deathPlace}
                                                            </span>
                                                        )}
                                                    </div>
                                                </div>
                                    ...
                                    {selectedTag.type === 'concept' && (
                                        <>
                                            <div>
                                                <span className="block text-xs text-slate-500 mb-1">Concept Flavor</span>
                                                <span className="text-white font-medium">{(selectedTag as ConceptTag).metadata?.flavor || 'General'}</span>
                                            </div>
                                            <div>
                                                <span className="block text-xs text-slate-500 mb-1">Cradle of Origin</span>
                                                <span className="text-white font-medium">{(selectedTag as ConceptTag).metadata?.cradlePlaceString || 'None Recorded'}</span>
                                            </div>
                                            {((selectedTag as ConceptTag).metadata?.startDate || (selectedTag as ConceptTag).metadata?.endDate) && (
                                                <div className="col-span-2 mt-2 pt-2 border-t border-slate-800/50">
                                                    <span className="block text-xs text-slate-500 mb-1">Chronology / Lifespan</span>
                                                    <span className="text-white font-medium text-sm">
                                                        {[(selectedTag as ConceptTag).metadata?.startDate, (selectedTag as ConceptTag).metadata?.endDate].filter(Boolean).join(' - ') || 'N/A'}
                                                    </span>
                                                </div>
                                            )}
                                        </>
    ...
    return (
        <div className="h-full flex flex-col bg-slate-950">
            ...
            <div className="p-6 border-b border-slate-800 flex justify-between items-center shrink-0 bg-slate-900/50 backdrop-blur-md z-10">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex bg-slate-900 p-1 rounded-lg border border-slate-800 overflow-x-auto custom-scrollbar max-w-md">
                        {['all', 'person', 'pet', 'place', 'event', 'thing', 'concept'].map(type => (
                            <button
                                key={type}
            ...
            {/* [ZEN] V'Ger Shield: Gedcom Inspector */}
            {/* [ZEN] V'Ger Shield: Gedcom Inspector */}
            {selectedTag && showInspector && selectedTag.type === 'person' && currentUser && (
                <GedcomInspector
                    currentPerson={selectedTag as PersonTag}
                    user={currentUser}
                    onClose={() => setShowInspector(false)}
                    onUpdateTag={(updatedPerson) => {
                        onReplaceTag(updatedPerson);
                        // Also update local selected tag so detail modal reflects changes immediately
                        setSelectedTag(updatedPerson);
