...
import { Edit2, Trash2, Smile, MessageSquare, MapPin, Tag as TagIcon, MoreHorizontal, Eye, ExternalLink, Loader2, Plus } from 'lucide-react';
...
import { DEFAULT_EMOJIS } from '../../types'; // [ZEN FIX]
import { GlassAvatar } from '../GlassAvatar';
import { formatLifeOSDate, getOrdinal, parseRobustDate } from '../../utils/dateSanitizer';
...
export const EventCard: React.FC<EventCardProps> = ({ event, media, tags, user, onEdit, onView, onTagClick, onMediaClick, onAddComment, onUpdateEvent, onDeleteEvent, onNavigate, onCreateTag, viewMode }) => {
    ...
    const matchingPlaceTag = useMemo(() => {
        ...
        return tags.find(t => {
            ...
            if (event.location?.coordinates && meta?.coordinates) {
                const dist = Math.abs(event.location.coordinates.lat - meta.coordinates.lat) + 
                             Math.abs(event.location.coordinates.lng - meta.coordinates.lng);
                if (dist < 0.0005) return true;
            }

            // Priority 2: Precise Address Match
            if (meta?.address?.streetAddress === event.location?.streetAddress) return true;

            // Priority 3: Name Match
            return t.name.toLowerCase() === event.location?.addressLocality?.toLowerCase();
        });
    }, [tags, event.location]);
    ...
    const dateObj = useMemo(() => {
        return parseRobustDate(event.date, [event.title, event.details, event.description || ''], user);
    }, [event.date, event.title, event.details, event.description, user]);
    ...
    const fullDateStr = formatLifeOSDate(event.date, event.datePrecision || 'day', user);
    ...
    const triggerHardDelete = async () => {
        if (!onDeleteEvent || isDeleting) return;
        setIsDeleting(true);
        try {
            const res = await fetch('/api/admin/deleteTimelineEvent', {
                method: 'POST',
            ...
            if (result.success) {
                ...
                alert('Pruning failed: ' + (result.error || 'Server error'));
            }
        } catch (err: any) {
            console.error("[Hard Delete] error:", err);
            alert('Pruning error: ' + err.message);
        } finally {
            setIsDeleting(false);
        }
    };
    ...
    const handlePromoteToTag = async (e: React.MouseEvent) => {
        e.stopPropagation();
        if (!event.location || !onCreateTag || isPromoting) return;
        
        setIsPromoting(true);
        try {
            const tagName = event.location.addressLocality || event.location.streetAddress || "New Place";
            const newTag = await onCreateTag(tagName, 'place', {
                address: event.location,
                coordinates: event.location.coordinates
            });
            if (newTag) {
                console.log("[Spatial Stitching] Promoted location to tag:", newTag.id);
                // Optionally navigate to the new tag immediately
                onTagClick(newTag);
            }
        } catch (err) {
    ...
    const backgroundOverlay = useMemo(() => {
        if (eventMedia.length === 1) {
            const mediaItem = eventMedia[0];
            const isVideo = getMediaType(mediaItem) === 'video';
            return (
                <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
                    {isVideo ? (
                        <video 
                            ...
                            playsInline 
                            autoPlay 
                            className="w-full h-full object-cover opacity-[0.35] blur-[80px] scale-[1.8] transform-gpu" 
                        />
                    ) : (
                        <img 
                    ...
                    <div className="absolute inset-0 bg-gradient-to-br from-[#0f1219]/50 via-transparent to-purple-900/30" />
                </div>
            );
        }
        return (
            <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-cyan-800/40 via-[#0f1219]/30 to-purple-800/50" />
                <div className="absolute -top-40 -right-40 w-[500px] h-[500px] bg-cyan-400/30 blur-[150px] rounded-full animate-pulse" style={{ animationDuration: '8s' }} />
                <div className="absolute -bottom-40 -left-40 w-[500px] h-[500px] bg-purple-500/40 blur-[150px] rounded-full animate-pulse" style={{ animationDuration: '10s' }} />
                <div className="absolute top-1/4 left-1/4 w-full h-full bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-cyan-500/10 via-transparent to-transparent blur-[180px]" />
                <div className="absolute bottom-1/4 right-1/4 w-full h-full bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-purple-500/10 via-transparent to-transparent blur-[180px]" />
            </div>
        );
    }, [eventMedia]);
        ...
        return (
            ...
            >
                {backgroundOverlay}
                 <div className="relative p-4 bg-gradient-to-b from-[#1a1d26] to-[#0f1219] border-b border-white/5 flex justify-between items-start">
                     <div className="overflow-hidden">
                        <h3 className="text-xl font-bold text-white whitespace-normal break-words leading-tight group-hover:text-cyan-400 transition-colors duration-300">{event.title || "Untitled"}</h3>
                        <div className="flex items-center gap-2">
                            <p className="text-[10px] text-cyan-500 font-mono tracking-wider">{fullDateStr}</p>
                            {event.location && (
                                <span className="text-[10px] text-emerald-500 font-mono tracking-wider flex items-center gap-1">
                                    <MapPin size={8} /> {event.location.addressLocality}
                                </span>
                            )}
                        </div>
                    </div>
                        ...
                        {onDeleteEvent && (
                            <div className="relative">
                                <button 
                                    ...
                                    title="Verify execution to permanently compile an atomic backup tombstone inside our secure ledger collection"
                                >
                                   <Trash2 size={14} />
                                </button>
                                {showDeletePopover && (
                                    <div className="absolute right-0 top-8 z-[100] w-64 bg-slate-900 border border-red-500/40 rounded-2xl shadow-2xl p-4 text-left font-sans select-none" onClick={(e) => e.stopPropagation()}>
                                        <p className="text-xs font-bold text-red-400 flex items-center gap-1.5 mb-2">
                                            ⚠️ Prune from Timeline?
                                        </p>
                                        <p className="text-[10px] text-slate-300 leading-relaxed mb-4">
                                            This will execute a permanent hard-delete and compile an atomic backup tombstone inside our secure ledger collection.
                                        </p>
                                        <div className="flex gap-2 justify-end">
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); setShowDeletePopover(false); }}
                                                className="px-2.5 py-1 text-[10px] bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-md font-bold transition-all"
                                                title="Cancel event pruning"
                                            >
                                                Cancel
                                            </button>
                                            <button 
                                                ...
                                                onClick={async (e) => { 
                                                    e.stopPropagation(); 
                                                    await triggerHardDelete(); 
                                                    setShowDeletePopover(false);
                                                }}
                                                className="px-2.5 py-1 text-[10px] bg-red-950 border border-red-500/40 text-red-400 hover:bg-red-900 rounded-md font-bold transition-all"
                                                title="Confirm permanent deletion to forensic ledger"
                                            >
                                                {isDeleting ? 'Purging...' : 'Yes, Purge Entry'}
                                            </button>
                                        </div>
                                    </div>
                                )}
                ...
                <div className="p-4 flex-grow flex flex-col">
                    <div className="text-sm text-slate-400 line-clamp-4 mb-4 flex-grow font-light leading-relaxed prose prose-sm prose-invert max-w-none">
    ...
    return (
        <div id={`event-card-${event.id}`} className="mb-6 relative group max-w-5xl mx-auto flex flex-col md:flex-row bg-[#0f1219] rounded-3xl border border-white/5 overflow-hidden shadow-xl hover:shadow-cyan-900/10 transition-all">
            ...
            <div 
                onClick={onView}
                className="md:w-32 bg-white/[0.02] backdrop-blur-sm border-b md:border-b-0 md:border-r border-white/5 p-4 flex flex-row md:flex-col items-center justify-between md:justify-center text-center cursor-pointer relative overflow-hidden"
                            ...
                            <div className="flex items-center gap-2 mt-2">
                                <button 
                                    onClick={(e) => { 
                                        e.stopPropagation(); 
                                        if (matchingPlaceTag) {
                                            onTagClick(matchingPlaceTag);
                                        } else {
                                            onNavigate && onNavigate('theMatrix', { view: 'map', focusLocation: event.location });
                                        }
                                    }}
                                    className={`flex items-center gap-2 px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all group/loc ${
                                        matchingPlaceTag 
                                        ? 'bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 hover:bg-cyan-500/20' 
                                        : 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/20'
                                    }`}
                                    title={matchingPlaceTag ? "View Place Tag Details" : "View on Interactive Map"}
                                >
                                    <MapPin size={12} className={`${matchingPlaceTag ? 'text-cyan-500' : 'text-emerald-500'} group-hover/loc:scale-110 transition-transform`} />
                                    {event.location.addressLocality}{event.location.addressRegion ? `, ${event.location.addressRegion}` : ''}
                                </button>

                                {!matchingPlaceTag && onCreateTag && (
                                    <button 
                                        onClick={handlePromoteToTag}
                                        disabled={isPromoting}
                                        className="p-1 text-slate-500 hover:text-emerald-400 transition-colors"
                                        title="Promote to Permanent Place Tag"
                                    >
                                        {isPromoting ? <Loader2 size={12} className="animate-spin" /> : <Plus size={14} />}
                                    </button>
                                )}
                            </div>
                         )}
                         {event.metadata?.importSource === 'blogspot' && (
                            <span className="text-[10px] font-bold text-orange-400 border border-orange-500/30 bg-orange-900/20 px-2 py-0.5 rounded-md mt-2 inline-block">
                                BLOGGER ARCHIVE
                            </span>
                         )}
                    </div>
                    ...
                    <div className="flex gap-2">
                        ...
                        {onDeleteEvent && (
                            <div className="relative">
                                ...
                                {showDeletePopover && (
                                    <div className="absolute right-0 top-10 z-[100] w-64 bg-slate-900 border border-red-500/40 rounded-2xl shadow-2xl p-4 text-left font-sans select-none" onClick={(e) => e.stopPropagation()}>
                                        <p className="text-xs font-bold text-red-400 flex items-center gap-1.5 mb-2">
                                            ⚠️ Prune from Timeline?
                                        </p>
                                        <p className="text-[10px] text-slate-300 leading-relaxed mb-4">
