...
const PersonConnections: React.FC<PersonConnectionsProps> = ({ tag, allTags, meta, handleChange, primaryCompanion }) => {
    ...
    return (
        <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700 animate-in fade-in overflow-hidden">
            ...
            <div className="space-y-2 mb-6 max-h-64 overflow-y-auto custom-scrollbar pr-2">
                {sortedRelationships.map((rel: any, idx: number) => {
                    ...
                    const relDisplayName = relative ? ((relative.type === 'person' && (relative as any).metadata?.displayName) ? (relative as any).metadata.displayName : relative.name) : 'Unknown Entity';
                    return (
                        <div key={idx} className="flex justify-between items-center bg-slate-900 p-2 rounded border border-slate-800 group">
                            <div className="flex items-center gap-2 flex-grow">
                                <span className="text-sm text-white font-bold truncate">{relDisplayName}</span>
                                <span className="text-[10px] text-slate-500 font-mono uppercase tracking-wide">is my</span>
                                <span className="text-xs font-bold bg-violet-900/30 text-violet-200 border border-violet-500/30 px-3 py-1 rounded-full text-center capitalize">{rel.type}</span>
                            </div>
            ...
            <div className="bg-slate-900 p-4 rounded-xl border border-slate-700/50 space-y-3">
                ...
                <div className="flex gap-2">
                    <select
                        ...
                        className="bg-slate-800 border border-slate-600 rounded px-3 py-2 text-sm text-white flex-1 min-w-0 focus:border-cyan-500 outline-none"
                    >
                        <option value="">Select Entity...</option>
                        {eligibleEntities.map(t => {
                            const dName = (t.type === 'person' && (t as any).metadata?.displayName) ? (t as any).metadata.displayName : t.name;
                            return (
                                <option key={t.id} value={t.id} label={`${dName} [${t.type.toUpperCase()}]`}>
                                    {dName} [{t.type.toUpperCase()}]
                                </option>
                            );
                        })}
                    </select>
