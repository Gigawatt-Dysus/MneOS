...
import { generateDaydreamEntry, generateDeepDiveEntry, generateDeepDiveFromQuery } from '../services/aiOrchestrator';
import { aiStateBridge } from '../utils/aiStateBridge';

export const useGigiAI = (
    // [STABILITY-SYNC-V2] Normalizing AI State Queue
    user: User | null, 
    events: LifeEvent[], 
    tags: Tag[], 
    media: Media[], 
    ...
    const handleTriggerDeepDive = useCallback(async (target: LifeEvent | string) => {
        ...
        if (typeof target === 'string') {
            ...
            eventId = target.id;
            try {
                // [ZEN FIX] Passed target.id (string) instead of target (LifeEvent object)
                // This matches the signature pattern confirmed in DeepDiveReporter.tsx
                result = await generateDeepDiveEntry(target.id, user, events, tags, media);
            } catch (e) {
                 console.error(e);
