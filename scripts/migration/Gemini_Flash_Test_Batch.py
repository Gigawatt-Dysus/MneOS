...
log_filename = f"C:\\MneOS\\scratch\\Vision_Batch_Log_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
sys.stdout = DualLogger(log_filename)
print(f"📝 Active Session Log initialized: {log_filename}\n")
...
# Using Gemini 3.1 Flash-Lite via Vertex AI SDK for a strict 100-image test batch
...
vertex_client = genai.Client(vertexai=True, project=PROJECT_ID, location="global")
...
def process_worker(q, collection):
    ...
    while True:
        ...
        try:
            ...
            prompt = "You are an objective computer vision archivist performing visual extraction. Analyze this image with strict literal precision."
            ...
            prompt += "\n\nCRITICAL INSTRUCTIONS:\n1. Jump straight into the description. DO NOT start with boilerplate.\n2. DO NOT repeat the filename.\n3. Provide a dense, highly literal, and objective visual description of the scene. Focus STRICTLY on undeniable visible objects, people, colors, text, and physical actions. Avoid all poetic language, emotional fluff, or scene-setting.\n4. Write a concise, information-rich paragraph.\n5. DO NOT hallucinate, invent details, or assume context outside the frame. If the background is blurry, ignore it."
            ...
            while True:
                try:
                    ...
                    if res.text is None:
                        caption = "[BLOCKED BY VERTEX AI SAFETY FILTERS]"
                    else:
                        caption = res.text.strip()
                    break # Success!
                except Exception as e:
                    err_str = str(e)
                    if "'NoneType' object has no attribute 'strip'" in err_str:
                        # Vertex AI SDK throws this internally when blocked by safety filters
                        caption = "[BLOCKED BY VERTEX AI SAFETY FILTERS]"
                        break
...
try:
    print("\n🚀 Running Gemini Flash TEST BATCH (100 images maximum)...")
    m_res = process_collection("pending_accessions")
    print("\n✅ Test batch complete. Review the outputs in the Matrix Gallery before doing a larger run.")
