...
async function fixDriveLetters() {
    console.log("ðŸš€ Starting vector swap in staging.db (D: -> F:)...");
    
    const queries = [
        `UPDATE files SET filepath = REPLACE(filepath, 'D:\\', 'F:\\') WHERE filepath LIKE 'D:\\%'`,
        `UPDATE airlock_jobs SET filepath = REPLACE(filepath, 'D:\\', 'F:\\') WHERE filepath LIKE 'D:\\%'`,
        `UPDATE files SET filepath = REPLACE(filepath, 'D:/', 'F:/') WHERE filepath LIKE 'D:/%'`,
        `UPDATE airlock_jobs SET filepath = REPLACE(filepath, 'D:/', 'F:/') WHERE filepath LIKE 'D:/%'`
    ];
