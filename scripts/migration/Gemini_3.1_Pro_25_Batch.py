...
# Using Gemini 2.5 Pro via Vertex AI SDK for a strict 25-image test batch
    ...
    while True:
        ...
        try:
            ...
            if parent_event:
                ...
                if context_chunk:
                    prompt += f"\n\nCONTEXT FROM JOURNAL ENTRY: '{context_chunk}'\n\nUse this context to inform your description of relationships, object colors, or locations if they appear in the image, but DO NOT hallucinate things from the text that are not actually visible."
            ...
            # Save the result to the NEW test bucket (collection)
            test_record = {
                "mediaId": doc_id,
            ...
            db["gemini_captioning_test"].insert_one(test_record)

            # Atomic update on the main collection to mark as processed
            update_data = {
                "aiProcessed": True,
                "aiModel": MODEL_NAME,
                "aiProcessedAt": now
...
def process_collection(collection_name):
    ...
    query = {
        "aiProcessed": False, 
        "claimed_by": None,
        "$or": [{"aiRetryCount": None}, {"aiRetryCount": {"$lt": 3}}]
    }
...
try:
    print("\n🚀 Running Gemini 3.1 Pro TEST BATCH (25 images maximum)...")
    m_res = process_collection("pending_accessions")
    print("\n✅ Test batch complete. Review the outputs in the `gemini_captioning_test` MongoDB collection before doing a larger run.")
