...
(function() {
    ...
    function getVisitedKey() {
        ...
        return `mneos_${platform.toLowerCase()}_visited_sessions`;
    }

    function getCrawlActiveKey() {
        const platform = detectPlatform();
        return `mneos_${platform.toLowerCase()}_crawl_active`;
    }
    ...
    let isNanoBananaActive = false;
    const sessionTimestampMap = {};

    function parseGeminiPayloads(text, url = '') {
        if (!text || typeof text !== 'string') return;
        try {
            // 1. Detect Nano Banana network tool/model calls dynamically
            const lower = text.toLowerCase();
            if (lower.includes('nano-banana') || lower.includes('flash-image') || lower.includes('pro-image') || lower.includes('imagen_3') || lower.includes('generate_image')) {
                console.log('[MneOS Harvester] 🍌 Nano Banana / Image Tool payload detected in network traffic!');
                isNanoBananaActive = true;
            }
            ...
            window.fetch = async function(...args) {
                ...
                try {
                    ...
                    if (url.includes('batchexecute') || url.includes('BardChatUi') || url.includes('bard')) {
                        ...
                        clone.text().then(t => parseGeminiPayloads(t, url)).catch(() => {});
                    }
                } catch(e) {}
                return response;
            };
                ...
                this.addEventListener('load', function() {
                    try {
                        if (this.responseURL && (this.responseURL.includes('batchexecute') || this.responseURL.includes('BardChatUi'))) {
                            parseGeminiPayloads(this.responseText, this.responseURL);
                        }
                    } catch(e) {}
                });
    ...
    function markVisited(idOrTitle) {
        const visited = getStorageJSON(getVisitedKey(), []);
        if (idOrTitle && !visited.includes(idOrTitle)) {
            visited.push(idOrTitle);
            setStorageJSON(getVisitedKey(), visited);
        }
    }
    ...
    function injectUI() {
        ...
        if (!triggerEl) {
            ...
            resetBtn.onclick = () => {
                localStorage.removeItem(getVisitedKey());
                localStorage.setItem(getCrawlActiveKey(), 'false');
                localStorage.removeItem('mneos_grok_visited_sessions');
                localStorage.setItem('mneos_grok_crawl_active', 'false');
                localStorage.removeItem('mneos_gemini_visited_sessions');
                localStorage.setItem('mneos_gemini_crawl_active', 'false');
                localStorage.removeItem(REHYDRATION_KEY);
                localStorage.removeItem(REHYDRATION_MSG_KEY);
                alert('MneOS visited session cache & crawl state cleared!');
                location.reload();
            };
        } else {
            const vLabel = document.getElementById('mneos-version-label');
            if (vLabel) vLabel.textContent = '⚡ v10.12.4';
        }
    ...
    function extractDOMTurns(platform) {
        if (platform === 'GEMINI') {
            // Target specific turn elements including modern Gemini DOM variations
            let rawNodes = Array.from(document.querySelectorAll(
                'user-query, model-response, [data-test-id*="user"], [data-test-id*="model"], .user-query-container, .model-response-container, chat-turn, [class*="user-query"], [class*="model-response"], .user-turn, .model-turn, [class*="query-content"], [class*="response-content"], message-content, .message-content, [class*="turn-content"]'
            ));

            // Fallback to searching inside main if specific selectors return empty
            if (rawNodes.length === 0) {
                const mainEl = document.querySelector('main') || document.querySelector('[role="main"]') || document.body;
                if (mainEl) {
                    rawNodes = Array.from(mainEl.querySelectorAll('.markdown, [class*="query"], [class*="response"], p, article, [class*="message"]'));
                }
            }
            ...
            if (geminiNodes.length > 0) {
                ...
                geminiNodes.forEach(node => {
                    ...
                    const isUser = tag.includes('user') || cls.includes('user') || attr.includes('user') || attr === 'user' || cls.includes('query');
    ...
    async function harvestCurrentSession(customTurns = null) {
        ...
        // Dynamic DOM & URL check for Nano Banana / Image generation mode
        const isUrlBanana = window.location.href.includes('/images') || window.location.search.includes('banana');
        const isDomBanana = !!document.querySelector('[aria-label*="Banana"], [data-tool-id*="image"], [class*="image-generation"]');
        const isMediaActive = isNanoBananaActive || isUrlBanana || isDomBanana;

        // Reset dynamic state for next card
        isNanoBananaActive = false;
    ...
    function toggleAutoCrawl() {
        ...
        localStorage.setItem(getCrawlActiveKey(), 'true');
        updateCrawlButtonState();
        startAutoCrawl();
    }

    function isValidChatLink(link, platform) {
        if (!link) return false;

        const rawHref = link.getAttribute('href') || link.href || '';
        const href = rawHref.toLowerCase();
        const label = (link.getAttribute('aria-label') || link.getAttribute('title') || link.innerText || '').toLowerCase();
        const parentClasses = (link.className + ' ' + (link.parentElement ? link.parentElement.className : '')).toLowerCase();

        // 1. Hard Exclude Profile / Account / Settings / Help / Activity / Billing / Upgrade
        if (href.includes('myaccount.google.com') || href.includes('accounts.google.com') || href.includes('/settings') || href.includes('/help') || href.includes('/activity') || href.includes('/upgrade')) return false;
        if (label.includes('google account') || label.includes('manage your') || label.includes('sign in') || label.includes('settings') || label.includes('help') || label.includes('feedback') || label.includes('activity') || label.includes('profile')) return false;
        if (link.closest('[class*="profile"], [class*="account"], [id*="account"], [id*="profile"], side-nav-bottom-container, .user-profile-button, [aria-label*="Google Account"]')) return false;
        if (parentClasses.includes('profile') || parentClasses.includes('account') || parentClasses.includes('settings')) return false;

        // 2. Platform-specific chat link validation
        if (platform === 'GEMINI') {
            let path = '';
            try {
            ...
            const hasChatParam = rawHref.includes('chat=');

            if (!hasAppSessionId && !hasChatParam) {
                return false;
            }
        ...
        if (platform === 'GROK') {
            return href.includes('/c/') || href.includes('/chat/');
        }

        if (platform === 'NOTEBOOK_LM') {
            return href.includes('/notebook/') || href.includes('/project/');
        }
    ...
    async function startAutoCrawl() {
        ...
        console.log('[MneOS Harvester] Sweep Complete.');
