...
import { convertTimestampsToDates, cleanForFirestore } from '../services/firebase/core';
import { generatePeerResponse } from '../services/ai/generators/peer';
...
export const usePeerChat = ({ user, sessionId, activeVertName, addToast }: UsePeerChatProps) => {
    ...
    useEffect(() => {
        ...
        const unsub = onSnapshot(q, (snapshot) => {
            ...
            const loaded = snapshot.docs.map(doc => convertTimestampsToDates({ id: doc.id, ...doc.data() }) as PeerChatSegment);
            setMessages(loaded);
