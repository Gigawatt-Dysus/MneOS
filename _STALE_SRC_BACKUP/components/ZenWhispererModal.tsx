...
const ZenWhispererModal: React.FC<ZenWhispererModalProps> = ({ isOpen, onClose, user }) => {
    ...
    const [health, setHealth] = useState<Record<string, ServiceState>>({
        GEMINI: { id: 'GEMINI', status: 'gray', message: 'Init...', latency: null, details: 'Waiting...' },
    ...
    const [config, setConfig] = useState<DevConfig>({
        coderName: 'Zen', basePrompt: '', geminiKey: '', grokKey: '', 
    ...
    const pingCloud = useCallback(async () => {
        const geminiStatus: ServiceState = { id: 'GEMINI', status: config.geminiKey ? 'green' : 'yellow', message: config.geminiKey ? 'Ready' : 'No Key', latency: null, details: 'API Key Configured' };
        ...
        setHealth(prev => ({ ...prev, GEMINI: geminiStatus, GROK: grokStatus }));
    }, [config]);
        ...
        try {
            ...
            if (model === 'gemini') {
                ...
                const res = await fetch("https://api.x.ai/v1/chat/completions", {
                    ...
                    body: JSON.stringify({ messages: [{ role: "system", content: `You are ${config.coderName}.` }, { role: "user", content: prompt }], model: "grok-beta", stream: false })
                });
