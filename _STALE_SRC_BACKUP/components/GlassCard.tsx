...
export const GlassCard: React.FC<GlassCardProps> = ({ 
    title, subtitle, image, isPrimary, onEdit, onDelete, className = "", onClick 
}) => {
    return (
        <div 
            onClick={onClick}
            className={`group relative p-4 rounded-2xl bg-black/20 border border-white/5 hover:bg-white/5 hover:border-white/10 hover:shadow-lg hover:shadow-cyan-900/10 transition-all duration-300 backdrop-blur-md flex items-center gap-4 cursor-pointer ${className}`}
        >
