...
import { Image as ImageIcon, Crop, Trash2, Wand2, X, Check, Upload } from 'lucide-react';
...
const ProfileEditor: React.FC<ProfileEditorProps> = (props) => {
    ...
    return (
        <div className="fixed inset-0 z-40 flex justify-center items-start pt-20 md:pt-32 px-2 md:px-4 pb-4 md:pb-8 bg-black/90 backdrop-blur-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                ...
                <MatrixSelector
                    ...
                    onSelect={(media) => {
                        if (activeTab === 'identity') {
                            setProfilePicPreview(media.url || media.thumbnailUrl);
                        }
            ...
            {/* --- MAIN EDITOR CARD --- */}
            <div className="w-full max-w-6xl h-full md:h-[calc(100vh-160px)] bg-[#0f1219] border border-white/10 rounded-2xl shadow-2xl flex flex-col md:flex-row overflow-hidden relative ring-1 ring-white/5">

                {/* LEFT SIDEBAR */}
                <div className="w-full md:w-72 bg-black/40 border-b md:border-b-0 md:border-r border-white/5 p-6 flex flex-col items-center shrink-0 overflow-y-auto custom-scrollbar">
                ...
                <div className="flex-1 flex flex-col min-w-0 bg-[#161e2a]">
                    ...
                    {/* Header / Tabs */}
                    <div className="h-16 border-b border-white/5 flex items-center px-4 md:px-8 bg-black/20 shrink-0 relative">
                        <div className="flex-1 flex gap-4 md:gap-8 h-full overflow-x-auto no-scrollbar mask-gradient-right">
                            {(['identity', 'gallery', 'controls'] as const).map(tab => (
                                <button
                                    key={tab}
                        ...
                        <button
                            ...
                            className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 p-2 text-slate-500 hover:text-white transition-colors shrink-0"
                        >
                            <X size={20} />
                        </button>
                    </div>
                    ...
                    {/* Scrollable Content */}
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-8 bg-[#161e2a]">
                    ...
                    {/* Footer Actions */}
                    <div className="h-20 border-t border-white/5 flex items-center justify-end px-4 md:px-8 gap-4 bg-[#161e2a] shrink-0">
                        <GlassButton
                            onClick={() => props.onNavigate('dashboard')}
                            variant="ghost"
