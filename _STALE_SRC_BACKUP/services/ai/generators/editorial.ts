import { callFireworks, callGemini, callXAI } from '../providers'; 
import { 
    getProviderForModel, 
    getFireworksKey, 
    getXAIKey, 
...
const executeRequest = async (provider: string, model: string, prompt: string) => {
    ...
    if (provider === 'fireworks') {
        ...
        const res = await callFireworks(model, [], prompt, key, config);
        return res.text || "";
    }
    if (provider === 'xai') {
        const key = getXAIKey();
        if (!key) throw new Error("Key Missing: xAI");
        const res = await callXAI(model, [{ role: 'user', parts: [{ text: prompt }] }], "System Logic");
        return res.text || "";
    }
    if (provider === 'google') {
        const key = getGeminiKey();
        if (!key) throw new Error("Key Missing: Gemini");
