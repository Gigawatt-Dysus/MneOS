...
export const MemoryService = {
    ...
    async recallContext(userQuery: string): Promise<string> {
        console.log(`%c[Cortex] Recalling memories for: "${userQuery}"...`, 'color: cyan');
    ...
    async remember(msg: ChatMessage, msgId: string) {
        if (msg.role === 'system') return;
        // Fire and forget - don't block UI
        indexMessage(msg, msgId).catch((err: any) => console.warn("[Cortex] Indexing failed", err));
    }
};